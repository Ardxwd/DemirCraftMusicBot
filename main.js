...
const animeCommandFiles = fs
  .readdirSync(animeCommandsPath)
  .filter((file) => file.endsWith(".js"));
const funCommandFiles = fs
  .readdirSync(funCommandsPath)
  .filter((file) => file.endsWith(".js"));
const utilityCommandFiles = fs
  .readdirSync(utilityCommandsPath)
  .filter((file) => file.endsWith(".js"));
const imageCommandFiles = fs
  .readdirSync(imageCommandsPath)
  .filter((file) => file.endsWith(".js"));
const basicCommandFiles = fs
  .readdirSync(basicCommandsPath)
  .filter((file) => file.endsWith(".js"));
...
async function login() {
  try {
    ...
    console.log(
      "\x1b[31m%s\x1b[0m",
      "❌ Client Not Login, Restarting Process...",
    );
    process.kill(1);
  }
}
...
client.once("ready", () => {
  setTimeout(() => {
    ...
    client.user.setPresence({
      activities: [{ name: `v1.8`, type: ActivityType.Streaming ,url: "https://www.twitch.tv/ardaz3" }],
      status: "dnd",
    });
  }, 2000);
});
...
setInterval(() => {
  if (!client || !client.user) {
    console.log(
      "\x1b[31m%s\x1b[0m",
      "❌ Client Not Logged in, Restarting Process...",
    );
    process.kill(1);
  }
}, 15000);
